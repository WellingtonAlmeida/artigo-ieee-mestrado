...
%se either SI (MKS) or CGS as primary units. (SI units are strongly
%ncouraged.) English units may be used as secondary units (in parentheses).
%his applies to papers in data storage. For example, write ``15
%b/cm$^{2}$ (100 Gb/in$^{2})$.'' An exception is when
%nglish units are used as identifiers in trade, such as ``3$^{1\!/\!2}$-in
%isk drive.'' Avoid combining SI and CGS units, such as current in amperes
%nd magnetic field in oersteds. This often leads to confusion because
%quations do not balance dimensionally. If you must use mixed units, clearly
%tate the units for each quantity in an equation.
%
%he SI unit for magnetic field strength $H$ is A/m. However, if you wish to use
%nits of T, either refer to magnetic flux density $B$ or magnetic field
%trength symbolized as $\mu _{0}H$. Use the center dot to separate
%ompound units, e.g., ``A$\cdot $m$^{2}$.''
%
%
%Figure[t!](topskip=0pt, botskip=0pt, midskip=0pt){fig1.png}
% \textbf{Magnetization as a function of applied field.
%t is good practice to explain the significance of the figure in the caption.}\label{fig1}}
The management of technical knowledge in legacy systems is a well-established challenge in software engineering. To identify the state of the art and existing gaps, a Systematic Literature Mapping (SLM) was conducted following the guidelines proposed by Kitchenham \cite{kitchenham}. The search was performed across three major databases (ACM Digital Library, IEEE Xplore, and Scopus), initially identifying 311 studies. After applying rigorous inclusion and exclusion criteria, 23 primary studies were selected for detailed analysis. The distribution of these studies and the identified research gaps are summarized in Table \ref{tab:slm_stats}

The SLM revealed that while 56\% of the studies (13 papers) address the documentation of legacy systems directly \cite{garousi}, and another 13 papers propose structured methodologies or frameworks, there is a clear trend toward high-complexity technical solutions. For instance, several studies focus on automated reverse engineering, such as the use of PageRank algorithms to identify key classes \cite{chirila} or Latent Dirichlet Allocation (LDA) to recover architectural layers \cite{belle}. While these automated approaches are valuable for technical recovery, they often overlook the human factor and the continuous ``friction'' that developers face during daily maintenance.
...
\begin{tabular}{|p{25pt}|p{145pt}|p{25pt}|p{35pt}|}
\hline
...
\begin{enumerate}
    ...
    \item Initial Structure: To reduce cognitive load, the repository was pre-populated with a flexible directory structure and reference examples. This structure served as a practical guide for the team on how to organize Markdown files and maintain content standards.
...
E11 & Hu et al. (2015) \cite{e15_hu} & Models the evolution of development topics using Dynamic Topic Models (DTM) to capture changes in software focus over time. \\ \hline
E12 & Scalabrino (2017) \cite{e12_scalabrino} & Recommends entry-point code artifacts using knowledge graphs to reduce comprehension effort for new developers. \\ \hline
E13 & Link et al. (2019) \cite{e13_link} & Proposes a responsibility-oriented architecture recovery method based on text classification. \\ \hline
...
E15 & Kadar et al. (2015) \cite{e15_kadar} & Uses ontologies and UML modeling to generate semantic summaries of source code for maintenance teams. \\ \hline
E16 & Leemans et al. (2018) \cite{e16_leemans} & Applies process mining to event logs to analyze and understand the behavior of complex legacy systems. \\ \hline
E17 & Alzahrani (2024) \cite{e17_alzahrani} & A systematic review of software documentation trends, highlighting the lack of prospective documentation models. \\ \hline
E18 & Al-Saiyd (2017) \cite{e18_alsaiyd} & Proposes a bottom-up static comprehension strategy to improve code readability and maintainer productivity. \\ \hline
E19 & Terry and Chandrasekar (2025) \cite{e19_terry} & Identifies engineering barriers to legacy system evolution and proposes formal assessment methods. \\ \hline
E20 & Javed et al. (2016) \cite{e20_javed} & Presents a pattern language for constructing and maintaining traceability links between architecture, requirements, and code. \\ \hline
...
E22 & Balde et al. (2024) \cite{e22_balde} & Develops dynamic visualization tools to generate UML sequence and class diagrams for legacy system comprehension. \\ \hline
E23 & L’Erario et al. (2020) \cite{e23_lerario} & Proposes a software maintenance process model for SMEs, focusing on knowledge recovery and turnover impact. \\ \hline
\end{tabular}
\end{table*}
...
\bibitem{chirila} C.-B. Chirila and I. Şora, ``An analysis on the optimization of the weights scheme for a rule based key class classifier in object-oriented systems,'' in \emph{Proc. 23rd Int. Conf. on System Theory, Control and Computing (ICSTCC)}, 2019, pp. 413--418.

\bibitem{belle} A. B. Belle, G. E. Boussaidi, and S. Kpodjedo, ``Combining lexical and structural information to reconstruct software layers,'' \emph{Information and Software Technology}, vol. 74, pp. 1--16, 2016.
...
\bibitem{e15_hu} J. Hu, X. Sun, D. Lo, and B. Li, ``Modeling the evolution of development topics using dynamic topic models,'' in \emph{Proc. 2015 IEEE 22nd Int. Conf. on Software Analysis, Evolution, and Reengineering (SANER)}, 2015, pp. 3--12.

\bibitem{e12_scalabrino} S. Scalabrino, ``On software odysseys and how to prevent them,'' in \emph{Proc. IEEE 2017 IEEE/ACM 39th Int. Conf. on Software Engineering Companion (ICSE-C)}, 2017, pp. 91--93.

\bibitem{e13_link} D. Link, P. Behnamghader, R. Moazeni, and B. Boehm, ``Recover and relax: concern-oriented software architecture recovery for systems development and maintenance,'' in \emph{Proc. 2019 IEEE/ACM Int. Conf. on Software and System Processes (ICSSP)}, 2019, pp. 64--73.

\bibitem{antoniol} G. Antoniol, G. Canfora, G. Casazza, A. D. Lucia, and E. Merlo, ``Recovering traceability links between code and documentation: a retrospective,'' \emph{IEEE Transactions on Software Engineering}, 2025.

\bibitem{e15_kadar} R. Kadar, S. M. Syed-Mohamad, and N. A. Rashid, ``Semantic-based extraction approach for generating source code summary towards program comprehension,'' in \emph{Proc. IEEE 2015 9th Malaysian Software Engineering Conference (MySEC)}, 2015, pp. 129--134.

\bibitem{e16_leemans} M. Leemans, W. M. van der Aalst, M. G. van den Brand, R. R. Schiffelers, and L. Lensink, ``Software process analysis methodology—a methodology based on lessons learned in embracing legacy software,'' in \emph{Proc. IEEE 2018 IEEE Int. Conf. on Software Maintenance and Evolution (ICSME)}, 2018, pp. 665--674.

\bibitem{e17_alzahrani} A. A. Alzahrani, ``Software systems documentation: A systematic review,'' \emph{International Journal of Advanced Computer Science \& Applications}, vol. 15, no. 8, 2024.

\bibitem{e18_alsaiyd} N. A. Al-Saiyd, ``Source code comprehension analysis in software maintenance,'' in \emph{Proc. IEEE 2017 2nd Int. Conf. on Computer and Communication Systems (ICCCS)}, 2017, pp. 1--5.

\bibitem{e19_terry} S. Terry and V. Chandrasekar, ``Systems engineering barriers to legacy system evolution: Legacy system assessment,'' \emph{Systems Engineering}, vol. 28, no. 2, pp. 207--223, 2025.

\bibitem{e20_javed} M. A. Javed, S. Stevanetic, and U. Zdun, ``Towards a pattern language for construction and maintenance of software architecture traceability links,'' in \emph{Proc. 21st European Conf. on Pattern Languages of Programs}, 2016, pp. 1--20.

\bibitem{garousi} V. Garousi, G. Garousi-Yusifoglu, V. Ruhe, G. Zhi, J. Moussavi, and B. Smith, ``Usage and usefulness of technical software documentation: An industrial case study,'' \emph{Information and Software Technology}, vol. 57, pp. 664--682, 2015.

\bibitem{e22_balde} K. Balde, L. Prasad, and R. Yadav, ``Visualizing complexity: A dynamic approach to reverse engineering,'' in \emph{Proc. IEEE 2024 Int. Conf. on Advances in Computing Research on Science Engineering and Technology (ACROSET)}, 2024, pp. 1--7.

\bibitem{e23_lerario} A. L’Erario, H. C. S. Thomazinho, and J. A. Fabri, ``An approach to software maintenance: A case study in small and medium-sized businesses IT organizations,'' \emph{International Journal of Software Engineering and Knowledge Engineering}, vol. 30, no. 05, pp. 603--630, 2020.
